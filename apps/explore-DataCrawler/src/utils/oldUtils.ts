...
function getToAmountFromUserAmount(
  ...
  return {
    sendAmount:toAmount_fee.toString(),
    tradeFee: gasFee_fix.toString(),
    withholdingFee: tradingFee.toString()
  };
}
...
export function getAmountToSend(
  ...
  if (typeof fromChainID!='number') {
    throw new Error('fromChainID type error');
  }
  if (typeof toChainID!='number') {
    throw new Error('toChainID type error');
  }
  ...
  const value = new BigNumber(rAmount).dividedBy(
    new BigNumber(10 ** fromTokenDecimals),
  )
  ...
  result.tradeFee = calcResult.tradeFee;
  result.withholdingFee = calcResult.withholdingFee;
  return result;
}
/**
 * @param chainId
