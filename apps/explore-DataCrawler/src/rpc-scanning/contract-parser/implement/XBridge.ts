...
export default class XBridge extends EVMPraser{
    async bridgeToV2(transaction: TransactionResponse, parsedData: TransactionDescription):Promise<TransferAmountTransaction[]> {
        ...
        const data = abiCoder.decode([ "address", "bytes" ], args[5]);
        console.log(data, '===')
        const txData = this.buildTransferBaseData(transaction, parsedData);
        ...
        const transfer = this.buildExecuteStatus([txData], null);
        return {} as any;
    }
}
